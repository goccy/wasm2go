...
func (t *translator) moduleGlobalOffsets() []int {
	...
	if t.excSlots > 0 {
		// excPending int32 + excTag uint32 + excVals [excSlots]uint64.
		// off is 8-aligned here and the int32/uint32 pair fills exactly
		// one word, so the uint64 array needs no padding.
		off += 8 + 8*t.excSlots
	}
...
// DirectAsmExcLayout is the byte offset of each exception-state field
// within the generated Module struct (see Result.DirectAsmExc).
...
// moduleExcOffsets computes the exception-state field offsets the
// direct-asm emitters hardcode, following the same field-order model
// as moduleGlobalOffsets. Nil when the module has no exception state.
func (t *translator) moduleExcOffsets() *DirectAsmExcLayout {
	if t.excSlots <= 0 {
		return nil
	}
	off := 0
	if len(t.mod.Memories) > 0 {
		off += 24 + 8 + 8 // memory []byte + maxMem uint64 + M unsafe.Pointer
	}
	if t.opts.OutlineMinValues > 0 {
		off += 128 * 8 // outlinePack [128]uint64
	}
	return &DirectAsmExcLayout{Pending: off, Tag: off + 4, Vals: off + 8}
}
...
// pack, the exception state, and each module-defined global). A
// drift between this package's real layout and the assumed offsets
// then fails the consumer's build instead of corrupting memory at
// run time.
func (t *translator) appendDirectAsmLayoutFile(files map[string][]byte) {
	...
	if exc := t.moduleExcOffsets(); exc != nil {
		assert(t.fieldName("excPending"), exc.Pending)
		assert(t.fieldName("excTag"), exc.Tag)
		assert(t.fieldName("excVals"), exc.Vals)
	}
	...
	if asserted == 0 {
		// Nothing to pin (no memory, no outline pack, no exception
		// state, no defined globals) — no offsets are hardcoded either.
		return
	}
	var f strings.Builder
	fmt.Fprintf(&f, "// Code generated by wasm2go. DO NOT EDIT.\n\n")
	fmt.Fprintf(&f, "package %s\n\nimport \"unsafe\"\n\n", pkg)
	fmt.Fprintf(&f, "// Compile-time pins for the Module field offsets direct-asm\n")
	fmt.Fprintf(&f, "// bodies hardcode. A nonzero index here means the layout moved.\n")
	fmt.Fprintf(&f, "var directAsmLayoutProbe Module\n\nvar (\n")
	f.WriteString(b.String())
	fmt.Fprintf(&f, ")\n")
	files[dir+"directasm_layout.go"] = []byte(f.String())
}
