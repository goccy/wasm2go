...
// TestInstrReaderPosAndSrc verifies the cursor-state accessors stay
// consistent as reads advance the reader. We don't assert on
// internal field layout — only on the publicly observed `Pos()` /
// `Src()` return values matching what we set up.
func TestInstrReaderPosAndSrc(t *testing.T) {
	src := []byte{0x05, 0xe5, 0x8e, 0x26}
	r := NewInstrReader(src)
	if got := r.Pos(); got != 0 {
		t.Errorf("initial Pos()=%d, want 0", got)
	}
	if got := r.Src(); &got[0] != &src[0] || len(got) != len(src) {
		t.Errorf("Src() did not return the underlying slice (len=%d, want %d)", len(got), len(src))
	}
	// Advance one byte and re-check.
	if _, err := r.ReadByte(); err != nil {
		t.Fatalf("ReadByte: %v", err)
	}
	if got := r.Pos(); got != 1 {
		t.Errorf("Pos() after one ReadByte=%d, want 1", got)
	}
	// Skip the rest via the LEB reader (3 bytes for 624485).
	if v, err := r.ReadU32(); err != nil || v != 624485 {
		t.Fatalf("ReadU32: got %d, err=%v", v, err)
	}
	if got := r.Pos(); got != len(src) {
		t.Errorf("Pos() after full read=%d, want %d", got, len(src))
	}
}
